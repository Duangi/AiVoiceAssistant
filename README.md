...
以下为在 rk3588 16GB 开发板上对 Zipformer 模型的流式推理基准测试示例（以项目自带测试脚本 `tests/hardware/test_asr_benchmark.py` 输出为准）：
...
----------------------------------------
🏎️  开始流式推理 (模拟真实数据流)...
   👉 Chunk 1: 耗时 172.62 ms
   👉 Chunk 2: 耗时 141.45 ms
   👉 Chunk 3: 耗时 132.19 ms
   👉 Chunk 4: 耗时 125.09 ms
   👉 Chunk 5: 耗时 125.39 ms
----------------------------------------
📊 性能分析报告
----------------------------------------
✅ 处理 Chunk 总数 : 5
...
🎵 原始音频时长     : 5.6115 秒
🚀 实时率 (RTF)     : 0.1246
🌟 结论: 达标！处理速度比说话快 8.0 倍
```

说明：以上数据为一次示例运行结果，环境为 rk3588 16GB 开发板；实际表现会随模型、NPU 驱动版本、系统负载和编译选项有所差异。建议在你的目标设备上多次跑基准以获得稳定指标。
